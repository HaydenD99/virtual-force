"""
Distributed UAV Deployment Algorithm Based on PSO Algorithm (加权版本)
基于分布式粒子群优化的无人机部署算法
...
class WeightedPSOOptimizer:
    ...
    def fitness_function(self, particle: np.ndarray, 
        ...
        try:
            ...
            # 适应度函数 (加权版本)
            # 优化目标：加权组合最小用户速率和系统和速率
            min_rate = rates.min()
            
            # 加权组合: min_rate + w_sum * sum_rate
            w_sum = self.config.get('w_sum_rate', 0.01)  # 系统和速率权重
...
def create_weighted_pso_config() -> Dict:
    """创建加权PSO配置"""
    config = create_distributed_pso_config()
    # 系统和速率的权重已在基础config中设置
    return config
...
if __name__ == "__main__":
    ...
    # 创建优化器
    optimizer = WeightedPSOOptimizer(config)
