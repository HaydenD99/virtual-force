"""
NewSSA优化器 - 基于论文"Deployment for NOMA-UAV Base Stations Based on Hybrid Sparrow Search Algorithm"
改进点：
...
class NewSSAOptimizer:
    ...
    def optimize(self, UE_pos: np.ndarray, ground_AP_pos: np.ndarray,
        ...
        # 真实 min_rate 指标（用于对外汇报/绘图），与 fitness 解耦（用于优化比较）
        min_rate_metrics = np.zeros(self.Z)
        
        for i in range(self.Z):
            uav_pos = self._decode_population(population[i])
            fit_res = self._compute_fitness(uav_pos, UE_pos, ground_AP_pos)
            # 兼容: _compute_fitness 可返回 (fitness, sum_rate, rates) 或 (fitness, sum_rate, rates, min_rate_metric)
            if isinstance(fit_res, tuple) and len(fit_res) == 4:
                fit, sum_rate, rates, mr_metric = fit_res
            else:
                fit, sum_rate, rates = fit_res
                mr_metric = fit
            fitness[i] = fit
            sum_rates[i] = sum_rate
            min_rate_metrics[i] = mr_metric
        ...
        best_min_rate_metric = min_rate_metrics[best_idx]
        ...
        for s in range(self.Iter_max):
            ...
            min_rate_metrics = min_rate_metrics[sorted_indices]
            ...
            if fitness[0] > best_fitness:
                ...
                best_min_rate_metric = min_rate_metrics[0]
                best_UAV_pos = self._decode_population(best_position)
            ...
            if fitness[current_best_idx] > best_fitness:
                ...
                best_min_rate_metric = min_rate_metrics[current_best_idx]
                best_UAV_pos = self._decode_population(best_position)
            
            # 记录历史
            history['iterations'].append(s)
            history['sum_rates'].append(best_sum_rate)
            history['min_rates'].append(best_min_rate_metric)
            ...
            if (s + 1) % 10 == 0:
                print(f"Iter {s+1:>3}/{self.Iter_max}: "
                      ...
                      f"Min={best_min_rate_metric:>7.4f}Mbps, "
        ...
        final_min_rate = best_min_rate_metric
