...
# def Calculate_SINR_and_SE_DL(signal, interference, B, gammaEqual, Pmax):
#     L = signal.shape[0]
#     K = signal.shape[1]
#
...
#
#         SE_MR_equal[k] = B*np.log2(1 + SINRnumerator/SINRdenominator)
#
#
#     return SE_MR_equal
def Calculate_SINR_and_SE_DL(signal, interference, B, gammaEqual, Pmax):
    ...
    for k in range(K):
        gamma_k = gammaEqual[k, :]  # (L,)
        SINRnumerator = np.abs(np.dot(signal[:, k], gamma_k)) ** 2

        SINRdenominator = 1.0 - SINRnumerator

        for i in range(K):
            gamma_i = gammaEqual[i, :]  # (L,)
            SINRdenominator += np.real(gamma_i.conj().T @ interference[:, :, k, i] @ gamma_i)
