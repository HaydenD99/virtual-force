"""
公平对比四个优化器的性能（加权Fitness版本）
- Balanced Virtual Force Optimizer V3 (不变)
- Weighted Genetic Algorithm Optimizer (加权fitness)
- Weighted PSO Optimizer (加权fitness)
- Weighted NewSSA Optimizer (加权fitness)

加权Fitness函数：fitness = min_rate + w_sum * sum_rate
...
from genetic_algorithm_optimizer_weighted import WeightedGeneticAlgorithmOptimizer, create_weighted_ga_config
from distributed_pso_optimizer_weighted import WeightedPSOOptimizer, create_weighted_pso_config
from newssa_optimizer_weighted import WeightedNewSSAOptimizer  # 使用加权NewSSA
...
def create_fair_configs(total_evaluations=1500, nbrOfRealizations=50, random_seed=44):
    ...
    config_newssa['w_sum_rate'] = 0.01  # 系统和速率权重
...
def run_fair_comparison(num_evaluations=1500, nbrOfRealizations=50, random_seed=44):
    ...
    optimizers_info = [
        ...
        ('GA', 'Weighted Genetic Algorithm', WeightedGeneticAlgorithmOptimizer),
        ('PSO', 'Weighted PSO', WeightedPSOOptimizer),
        ('NewSSA', 'Weighted NewSSA', WeightedNewSSAOptimizer)
    ]
