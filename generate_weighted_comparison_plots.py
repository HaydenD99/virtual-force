"""
UAV配置对比可视化 - 加权Fitness版本
对比6/9/12 UAVs配置下的性能（加权fitness = min_rate + 0.01 * sum_rate）
"""
...
# 6 UAVs数据 (加权版本)
with open('result/weighted_6uav_seed51.json', 'r') as f:
    data['6UAV'] = json.load(f)

# 9 UAVs数据 (加权版本，从weighted_partial_8.json)
data['9UAV'] = {
    "initial": {
        "min_rate": 31.593229480255477,
    ...
    "GA": {
        "min_rate": 34.227664144560165,
        "sum_rate": 2610.3195104600063,
    },
    "PSO": {
    ...
    "NewSSA": {
        "min_rate": 34.930312535636844,
        "sum_rate": 2676.7602012460575,
    }
}

# 12 UAVs数据 (加权版本)
with open('result/weighted_12uav_seed51.json', 'r') as f:
    data['12UAV'] = json.load(f)

# Algorithm names and colors
algorithms = {
    'VF': 'Balanced Virtual Force',
    'GA': 'Genetic Algorithm (Weighted)',
    'PSO': 'PSO (Weighted)',
    'NewSSA': 'NewSSA (Weighted)'
}
...
# ============================================================================
# Figure 1: Minimum Rate Comparison (Weighted Version)
# ============================================================================
...
ax1.set_title('(a) Minimum User Rate Comparison - Weighted Fitness', fontweight='bold')
...
ax2.set_title('(b) Minimum Rate Improvement - Weighted Fitness', fontweight='bold')
...
plt.tight_layout()
plt.savefig('result/weighted_minimum_rate_comparison.png', dpi=300, bbox_inches='tight')
print("✓ Saved: result/weighted_minimum_rate_comparison.png")
plt.close()

# ============================================================================
# Figure 2: Sum Rate Comparison (Weighted Version)
# ============================================================================
...
ax1.set_title('(a) System Sum Rate Comparison - Weighted Fitness', fontweight='bold')
ax1.set_xticks(x + width * 1.5)
ax1.set_xticklabels([f'4AP + {cfg}' for cfg in configs])
ax1.legend(loc='upper left', fontsize=8)
ax1.grid(axis='y', alpha=0.3, linestyle='--')
...
ax2.set_title('(b) Sum Rate Improvement - Weighted Fitness', fontweight='bold')
ax2.set_xticks(x + width * 1.5)
ax2.set_xticklabels([f'4AP + {cfg}' for cfg in configs])
ax2.legend(loc='upper left', fontsize=8)
ax2.grid(axis='y', alpha=0.3, linestyle='--')
...
plt.tight_layout()
plt.savefig('result/weighted_sum_rate_comparison.png', dpi=300, bbox_inches='tight')
print("✓ Saved: result/weighted_sum_rate_comparison.png")
plt.close()

# ============================================================================
# Figure 3: Combined Performance Metrics (Weighted Version)
# ============================================================================
...
ax1.set_ylabel('Minimum User Rate (Mbps)', fontweight='bold')
ax1.set_title('(a) Minimum Rate vs UAV Number - Weighted Fitness', fontweight='bold')
...
ax2.set_title('(b) Sum Rate vs UAV Number - Weighted Fitness', fontweight='bold')
...
ax3.set_title('(c) Minimum Rate Absolute Gain - Weighted Fitness', fontweight='bold')
...
ax4.set_title('(d) Sum Rate Absolute Gain - Weighted Fitness', fontweight='bold')
...
plt.suptitle('Performance Comparison - Weighted Fitness (min_rate + 0.01×sum_rate)',
            fontsize=14, fontweight='bold', y=0.995)
plt.savefig('result/weighted_comprehensive_performance_comparison.png', dpi=300, bbox_inches='tight')
print("✓ Saved: result/weighted_comprehensive_performance_comparison.png")
plt.close()
...
# ============================================================================
print("\n" + "="*80)
print("All weighted fitness comparison plots generated successfully!")
print("="*80)
print("\nGenerated files:")
print("  1. weighted_minimum_rate_comparison.png")
print("  2. weighted_sum_rate_comparison.png")
print("  3. weighted_comprehensive_performance_comparison.png")
print("\nLocation: result/")
print("="*80)

# Print data summary
print("\n" + "="*80)
print(" Weighted Fitness Performance Summary ".center(80))
print("="*80)
print("\n💡 Fitness Function: min_rate + 0.01 × sum_rate")
print("   (BVF uses original fitness, GA/PSO/NewSSA use weighted fitness)")

for cfg in configs:
    print(f"\n{'='*80}")
    print(f" Configuration: 4 Ground APs + {cfg} ".center(80))
    print('='*80)
    print(f"\n{'Algorithm':<30} {'Min Rate':<12} {'Sum Rate':<12} {'Min Δ%':<12} {'Sum Δ%':<12}")
    print("-" * 80)
    
    initial_min = data[cfg]['initial']['min_rate']
    initial_sum = data[cfg]['initial']['sum_rate']
    
    print(f"{'Initial':<30} {initial_min:<12.2f} {initial_sum:<12.0f} {'-':<12} {'-':<12}")
    ...
    for alg_key, alg_name in algorithms.items():
        ...
        print(f"{alg_name:<30} {min_rate:<12.2f} {sum_rate:<12.0f} "
              f"{min_improve:>+10.1f}% {sum_improve:>+10.1f}%")

print("\n" + "="*80)
print(" Comparison: Original vs Weighted Fitness ".center(80))
print("="*80)

# Load original data for comparison
with open('result/original_6uav_seed51.json', 'r') as f:
    orig_6uav = json.load(f)
with open('result/original_12uav_seed51.json', 'r') as f:
    orig_12uav = json.load(f)

orig_9uav = {
    "GA": {"min_rate": 34.61927199968264},
    "PSO": {"min_rate": 34.884425617079174},
    "NewSSA": {"min_rate": 31.01316688365497}
}

print("\n📊 Min Rate Comparison (Original vs Weighted):\n")
for cfg_idx, cfg in enumerate(configs):
    ...
    if cfg == '6UAV':
        ...
        orig_data = orig_12uav
    
    for alg in ['GA', 'PSO', 'NewSSA']:
        orig_val = orig_data[alg]['min_rate']
        weighted_val = data[cfg][alg]['min_rate']
        diff = weighted_val - orig_val
        print(f"    {alg:8}: {orig_val:6.2f} → {weighted_val:6.2f} ({diff:+6.2f} Mbps)")
    print()
