"""
UAV配置对比可视化 - 种子71
对比6/9/12 UAVs配置下的性能（原始fitness = min_rate）
"""
...
# 6 UAVs数据 (种子71)
with open('result/original_6uav_seed71.json', 'r') as f:
    data['6UAV'] = json.load(f)

# 9 UAVs数据 (种子71，从seeds_66_76_partial_11.json)
with open('result/seeds_66_76_partial_11.json', 'r') as f:
    all_seeds = json.load(f)
    data['9UAV'] = all_seeds['seed_71']

# 12 UAVs数据 (种子71)
with open('result/original_12uav_seed71.json', 'r') as f:
...
# ============================================================================
# Figure 1: Minimum Rate Comparison (Seed 71)
# ============================================================================
...
ax1.set_title('(a) Minimum User Rate Comparison - Seed 71', fontweight='bold')
...
ax2.set_title('(b) Minimum Rate Improvement - Seed 71', fontweight='bold')
...
plt.tight_layout()
plt.savefig('result/seed71_minimum_rate_comparison.png', dpi=300, bbox_inches='tight')
print("✓ Saved: result/seed71_minimum_rate_comparison.png")
plt.close()

# ============================================================================
# Figure 2: Sum Rate Comparison (Seed 71)
# ============================================================================
...
ax1.set_title('(a) System Sum Rate Comparison - Seed 71', fontweight='bold')
ax1.set_xticks(x + width * 1.5)
ax1.set_xticklabels([f'4AP + {cfg}' for cfg in configs])
ax1.legend(loc='upper left', fontsize=8)
ax1.grid(axis='y', alpha=0.3, linestyle='--')
...
ax2.set_title('(b) Sum Rate Improvement - Seed 71', fontweight='bold')
ax2.set_xticks(x + width * 1.5)
ax2.set_xticklabels([f'4AP + {cfg}' for cfg in configs])
ax2.legend(loc='upper left', fontsize=8)
ax2.grid(axis='y', alpha=0.3, linestyle='--')
...
plt.tight_layout()
plt.savefig('result/seed71_sum_rate_comparison.png', dpi=300, bbox_inches='tight')
print("✓ Saved: result/seed71_sum_rate_comparison.png")
plt.close()

# ============================================================================
# Figure 3: Combined Performance Metrics (Seed 71)
# ============================================================================
...
ax1.set_ylabel('Minimum User Rate (Mbps)', fontweight='bold')
ax1.set_title('(a) Minimum Rate vs UAV Number - Seed 71', fontweight='bold')
...
ax2.set_title('(b) Sum Rate vs UAV Number - Seed 71', fontweight='bold')
...
ax3.set_title('(c) Minimum Rate Absolute Gain - Seed 71', fontweight='bold')
...
ax4.set_title('(d) Sum Rate Absolute Gain - Seed 71', fontweight='bold')
...
plt.suptitle('Performance Comparison - Seed 71 (Original Fitness: min_rate)',
            fontsize=14, fontweight='bold', y=0.995)
plt.savefig('result/seed71_comprehensive_performance_comparison.png', dpi=300, bbox_inches='tight')
print("✓ Saved: result/seed71_comprehensive_performance_comparison.png")
plt.close()
...
# ============================================================================
print("\n" + "="*80)
print("All seed 71 comparison plots generated successfully!")
print("="*80)
print("\nGenerated files:")
print("  1. seed71_minimum_rate_comparison.png")
print("  2. seed71_sum_rate_comparison.png")
print("  3. seed71_comprehensive_performance_comparison.png")
print("\nLocation: result/")
print("="*80)

# Print data summary
print("\n" + "="*80)
print(" Seed 71 Performance Summary ".center(80))
print("="*80)
...
print("\n" + "="*80)
print(" Best Algorithm by Metric ".center(80))
print("="*80)

for cfg in configs:
    print(f"\n{cfg}:")
    
    # Find best for min rate
    best_min_alg = max(algorithms.keys(), key=lambda k: data[cfg][k]['min_rate'])
    best_min_val = data[cfg][best_min_alg]['min_rate']
    best_min_improve = ((best_min_val - data[cfg]['initial']['min_rate']) / 
                        data[cfg]['initial']['min_rate']) * 100
    
    # Find best for sum rate
    best_sum_alg = max(algorithms.keys(), key=lambda k: data[cfg][k]['sum_rate'])
    best_sum_val = data[cfg][best_sum_alg]['sum_rate']
    best_sum_improve = ((best_sum_val - data[cfg]['initial']['sum_rate']) / 
                        data[cfg]['initial']['sum_rate']) * 100
    
    print(f"  🏆 Min Rate: {algorithms[best_min_alg]} - {best_min_val:.2f} Mbps ({best_min_improve:+.1f}%)")
    print(f"  🏆 Sum Rate: {algorithms[best_sum_alg]} - {best_sum_val:.0f} Mbps ({best_sum_improve:+.1f}%)")
