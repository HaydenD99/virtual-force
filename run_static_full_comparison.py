...
计算时间对比基准测试
====================
静态场景: 从已有 raw_results.json 读取 100 个种子的计时数据
动态场景: 运行专项计时实验 (N_BENCH 个种子 × T_STEPS 步)，
          记录每步重新部署所需的优化时间

静态 : K=40, L=6, G=9
动态 : K=30, L=9, G=6

输出 : result/computation_time/
  - fig_time_static.png/pdf/svg     -- 静态部署计算时间柱状图
  - fig_time_dynamic.png/pdf/svg    -- 动态每步重部署计算时间柱状图
  - fig_time_combined.png/pdf/svg   -- 合并双图
  - fig_time_realworld.png/pdf/svg  -- 真实场景柱状图（若存在 time_realworld.json）
  - time_summary.json               -- 原始计时数据
"""
...
def _save_fig_multi(fig, out_path_no_ext):
    for ext in ['png', 'pdf', 'svg']:
        fig.savefig(f'{out_path_no_ext}.{ext}', dpi=900, bbox_inches='tight')
...
def plot_all(static_times, dyn_times, out_dir):
    ...
    plot_time_bar(
        ...
        '静态部署计算时间\n(K=40, L=6, G=9, 100 个种子)',
        '计算时间 (s)', ax, scenario='static'
    )
    plt.tight_layout()
    ...
    plot_time_bar(
        ...
        f'动态重部署单步计算时间\n(K=30, L=9, G=6, {N_BENCH}×{T_STEPS} 步)',
        '单步计算时间 (s)', ax, scenario='dynamic'
    )
    plt.tight_layout()
    ...
    plot_time_bar(
        STATIC_ALGS, static_times,
        '(a) 静态部署',
        '计算时间 (s)', axes[0], scenario='static'
    )
    plot_time_bar(
        DYNAMIC_ALGS, dyn_times,
        '(b) 动态单步重部署',
        '单步计算时间 (s)', axes[1], scenario='dynamic'
    )

    fig.suptitle('算法计算时间对比', fontsize=13, fontweight='bold', y=1.01)
    plt.tight_layout()
...
def plot_from_realworld_json(json_path, out_dir):
    ...
    ax.set_title('真实场景计算时间对比', fontsize=12, fontweight='bold', pad=8)
    ax.yaxis.grid(True, linestyle='--', alpha=0.5, zorder=0)
    ax.set_axisbelow(True)
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
...
if __name__ == '__main__':
    ...
    if os.path.exists(realworld_json):
        ...
        print(f"\n结果目录: {OUT_DIR}/")
        print("  fig_time_realworld.png/pdf/svg")
        raise SystemExit(0)
    ...
    summary = {
        'static': {
            a: {'mean': float(np.mean(v)), 'std': float(np.std(v)), 'samples': len(v)}
    ...
    print("  fig_time_static.png/pdf/svg")
    print("  fig_time_dynamic.png/pdf/svg")
    print("  fig_time_combined.png/pdf/svg")
