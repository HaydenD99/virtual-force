"""
平衡虚拟力优化器 - V4.1 (SA 稳健改进版)
修复说明：
1. 继承 V3 的历代最优记忆和智能重启机制
2. 优化 SA 权重演进曲线，防止全局引力消失过快
3. 精确控制温度相关的随机扰动
"""
...
class BalancedVirtualForceOptimizerV4:
    """平衡虚拟力优化器 - V4.1 (Simulated Annealing + V3 Robustness)"""
    ...
    def setup_parameters(self):
        ...
        # 权重演进边界 - 修复版
        self.weight_bounds = {
            'w_min_rate':    (0.35, 0.75), # 后期极度关注最差用户
            'w_universal':   (0.35, 0.15), # 初始强引力，后期保留背景引力
            'w_cooperation': (0.10, 0.05), # 后期减少协作力，增加精准度
            'w_separation':  (0.10, 0.03), # 后期允许 UAV 靠得稍近以服务极端用户
            'w_boundary':    (0.10, 0.02)  
        }
    ...
    def _get_current_weights(self, iteration: int) -> Dict:
        """根据退火进度计算当前权重"""
        # 使用 sigmoid 风格的平滑过渡
        progress = iteration / self.max_iterations
        alpha = 1.0 / (1.0 + np.exp(-10 * (progress - 0.5))) # 0->1 的 S 曲线
        
        current_weights = {}
        for key, (start, end) in self.weight_bounds.items():
            current_weights[key] = start + (end - start) * alpha
    ...
    def optimize(self, UE_pos: np.ndarray, ground_AP_pos: np.ndarray, UAV_pos: np.ndarray) -> Dict:
        """执行修复后的 V4 优化"""
        print("🚀 开始平衡虚拟力优化 V4.1 (SA + V3 Robustness Memory)...")
        ...
        for iteration in range(self.max_iterations):
            ...
            # 3. 智能重启 (核心修复)
            if no_improvement_count >= self.restart_threshold and restart_count < self.max_restarts:
                print(f"  🔄 触发智能重启 #{restart_count+1}")
                current_UAV_pos = best_UAV_pos.copy() + np.random.normal(0, self.perturbation_strength, (self.L, 3))
                current_UAV_pos[:, 2] = self.heights['UAV'] # 保持高度
                no_improvement_count = 0
                restart_count += 1
                continue
            ...
            # 5. 力计算与更新
            forces = self._compute_balanced_forces(UE_pos, ground_AP_pos, current_UAV_pos, rates, mask, betas, weights)
            current_UAV_pos = self._update_positions_sa(current_UAV_pos, forces, iteration, weights)
            
            history['min_rates'].append(min_rate)
            history['iterations'].append(iteration)
            
            if iteration % 10 == 0:
                print(f"Iter {iteration:2d} | MinRate: {min_rate:6.4f} | Best: {best_min_rate:6.4f} | w_min: {weights['w_min_rate']:.2f}")
        ...
        return {
            ...
            'history': {'min_rates': history['min_rates']}
        }

    def _compute_balanced_forces(self, UE_pos, ground_AP_pos, UAV_pos, rates, mask, betas, weights):
        forces = np.zeros((self.L, 3))
        # 复用 V3 核心计算逻辑，应用动态权重
        f_min = self._calc_min_rate_force(UE_pos, UAV_pos, rates, mask[:, self.G:])
        f_univ = self._calc_univ_force(UE_pos, UAV_pos)
        f_coop = self._calc_coop_force(ground_AP_pos, UAV_pos)
        f_sep = self._calc_sep_force(UAV_pos)
        
        forces += weights['w_min_rate'] * f_min
        forces += weights['w_universal'] * f_univ
        forces += weights['w_cooperation'] * f_coop
        forces += weights['w_separation'] * f_sep
        return forces

    def _update_positions_sa(self, UAV_pos, forces, iteration, weights):
        # 温度控制步长和扰动
        temp = self.T0 * (self.cooling_rate ** iteration)
        adaptive_step = self.step_size * (0.4 + 0.6 * temp)
        
        force_norms = np.linalg.norm(forces[:, :2], axis=1)
        max_f = np.max(force_norms) if np.max(force_norms) > 0 else 1
        displacement = adaptive_step * (forces[:, :2] / max_f)
        
        # 更加温和的 SA 扰动 (修复点)
        perturb_scale = 5.0 * temp
        perturbation = np.random.normal(0, perturb_scale, (self.L, 2))
        
        new_pos = UAV_pos.copy()
        new_pos[:, :2] += displacement + perturbation
    ...
    def _calc_min_rate_force(self, UE_pos, UAV_pos, rates, mask_uav):
        forces = np.zeros((self.L, 3))
        critical = np.where(rates <= np.min(rates) * 1.15)[0]
        for l in range(self.L):
            for k in critical:
                if mask_uav[k, l]:
                    dir = UE_pos[k] - UAV_pos[l]
                    dist = np.linalg.norm(dir) + 1e-6
                    forces[l, :2] += (self.force_params['K_min_rate'] / (dist + 5)) * (dir[:2] / dist)
        return forces
    ...
    def _calc_univ_force(self, UE_pos, UAV_pos):
        ...
        for l in range(self.L):
            for k in range(self.K):
                dir = UE_pos[k] - UAV_pos[l]
                dist = np.linalg.norm(dir) + 1e-6
                forces[l, :2] += (self.force_params['K_universal'] / (dist + 50)) * (dir[:2] / dist)
        return forces
    ...
    def _calc_coop_force(self, ground_AP_pos, UAV_pos):
        ...
        for l in range(self.L):
            for g in range(self.G):
                dir = ground_AP_pos[g] - UAV_pos[l]
                dist = np.linalg.norm(dir[:2]) + 1e-6
                if dist > self.force_params['cooperation_distance']:
                    forces[l, :2] += self.force_params['K_cooperation'] * (dir[:2] / dist)
        return forces

    def _calc_sep_force(self, UAV_pos):
        forces = np.zeros((self.L, 3))
        for i in range(self.L):
            for j in range(self.L):
                if i == j: continue
                dir = UAV_pos[i] - UAV_pos[j]
                dist = np.linalg.norm(dir[:2]) + 1e-6
                if dist < self.force_params['separation_distance']:
                    forces[i, :2] += self.force_params['K_separation'] * (dir[:2] / dist)
        return forces
