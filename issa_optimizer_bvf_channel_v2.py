"""
基于BVF信道模型的ISSA优化器（严格遵循论文公式）
- 优化目标: 最大化最小用户速率
- 优化变量: UAV的2D位置（xy），高度固定
- 改进策略: 混沌初始化 + Cauchy-Gaussian变异
"""
...
class ISSAOptimizerBVFChannel:
    ...
    def setup_parameters(self):
        ...
        self.pd = self.config.get('issa_pd', 0.2)  # 生产者比例 Pd
        self.sd = self.config.get('issa_sd', 0.2)  # 警戒者比例 Sd
        self.st = self.config.get('issa_st', 0.8)  # 安全阈值 ST
        
        # 2D边界约束（只优化xy位置）
        self.ub = np.array([self.square_length - 50, self.square_length - 50])  # 上界
        self.lb = np.array([50, 50])  # 下界
    ...
    def _decode_population(self, population: np.ndarray) -> np.ndarray:
        """
        解码种群为UAV 3D位置（添加固定高度）
        
        Parameters:
        -----------
        population : (L×2,) 或 (n, L×2)
        ...
        for i in range(n):
            for l in range(self.L):
                ...
                UAV_pos[i, l, 2] = self.UAV_height  # 固定高度
        
        return UAV_pos[0] if n == 1 else UAV_pos
    
    def _encode_UAV_pos(self, UAV_pos: np.ndarray) -> np.ndarray:
        """将UAV 3D位置编码为种群个体（提取xy）"""
        if UAV_pos.ndim == 2:  # (L, 3)
            return UAV_pos[:, :2].flatten()  # 只保留xy
        else:  # (n, L, 3)
    ...
    def _compute_fitness(self, UAV_pos: np.ndarray, UE_pos: np.ndarray, 
        ...
        try:
            ...
            # 适应度 = 最小用户速率（论文中的fitness function）
            fitness = min_rate
            fitness = max(fitness, 1e-6)
    ...
    def _update_producer(self, iter: int, population: np.ndarray, 
    ...
    def _cauchy_gaussian_mutation(self, best_position: np.ndarray, iter: int, 
        ...
        """
        Cauchy-Gaussian变异策略 - 论文公式(19)(20)
        
        Z_best^t = X_best^t · [1 + (1 - t²/iter_max²)·cauchy(0,σ²) 
                                 + t²/iter_max²·gauss(0,σ²)]
        
        σ = {
            1,                                      if f(X_best) > f(X_i)
            exp((f(X_best) - f(X_i)) / |f(X_best)|), otherwise
        }
        """
        # 计算自适应方差 σ（公式20）
        ...
        # 时间比率
        t = iter + 1
        t_ratio_sq = (t / self.max_iter) ** 2  # t²/iter_max²
        
        # Cauchy分布随机数
        cauchy_term = np.random.standard_cauchy(len(best_position)) * sigma
        cauchy_term = np.clip(cauchy_term, -5, 5)  # 限制长尾
        ...
        mutated_position = best_position * (
            1.0 + 
            (1 - t_ratio_sq) * cauchy_term * 0.1 +  # 早期：Cauchy主导
            t_ratio_sq * gaussian_term * 0.1          # 后期：Gaussian主导
        )
    ...
    def optimize(self, UE_pos: np.ndarray, ground_AP_pos: np.ndarray,
        ...
        Algorithm 1: ISSA-Enabled Multi-UAV Deployment
        """
        print("="*80)
        print("开始ISSA优化（严格遵循论文公式）...")
        print(f"  • 优化目标: 最大化最小用户速率")
        print(f"  • 优化变量: UAV 2D位置 (xy)，高度固定={self.UAV_height}m")
        print(f"  • 麻雀数量: {self.n_sparrows}")
        print(f"  • 最大迭代: {self.max_iter}")
        print(f"  • 生产者比例: {self.pd:.0%}, 警戒者比例: {self.sd:.0%}")
        print("="*80)
        ...
        # 步骤1: 混沌策略初始化（论文Algorithm 1, Line 5）
        print("\n[1/5] 混沌策略初始化种群（公式17-18）...")
        ...
        # 步骤3: 迭代优化（论文Algorithm 1, Line 7-24）
        ...
        for iter in range(self.max_iter):
            ...
            # Line 15-17: 更新警戒者（公式16）
            population = self._update_watch(population, best_position, fitness, 
            ...
            # Line 18-22: Cauchy-Gaussian变异（公式19-20）
