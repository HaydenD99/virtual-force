...
动态多配置对比 v2
=================
固定 L=6 (UAV), G=9 (地面AP), 变化 K=20/30/40
(G+L)*M/K = 60/K → 3.0 / 2.0 / 1.5

输出: result/multiconfig2/dynamic_K20.json
      result/multiconfig2/dynamic_K30.json
      result/multiconfig2/dynamic_K40.json
"""
import numpy as np
...
def _dyn_worker(args):
    ...
    # G=9: 3×3 grid
    gx = np.linspace(200, 800, 3); gy = np.linspace(200, 800, 3)
    GX, GY = np.meshgrid(gx, gy)
    gAP = np.column_stack([GX.flatten()[:G], GY.flatten()[:G], np.ones(G)*15.0])
    # L=6: 2×3 grid
    ux = np.linspace(200, 800, 3); uy = np.linspace(333, 667, 2)
...
if __name__ == '__main__':
    ...
    for K in K_LIST:
        ratio = (G+L)*4/K
        print(f"\n=== Dynamic  K={K}, L={L}, G={G}  (G+L)*M/K={ratio:.2f} ===")
        t0 = time.time()
        ...
        with mp.Pool(N_WORKERS) as pool:
            for i, (seed, res) in enumerate(
                ...
                print(f"  [{i:2d}/{N_SEEDS}] seed={seed}  "
                      f"DLB: mr={res['Dynamic LB-BVF']['min_rate']:.1f} "
                      f"jfi={res['Dynamic LB-BVF']['jfi']:.3f}", flush=True)
        ...
        out_path = os.path.join(OUT_DIR, f'dynamic_K{K}.json')
